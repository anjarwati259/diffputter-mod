...
# ===========================================================================
#  Redesigned Supervised Learnable Embedding — Perbaikan Lengkap v2
#
...
#
#  [4] alpha=0.3, beta=1.0 → balance antara rekonstruksi dan klasifikasi
#      (sebelumnya alpha=0.1 terlalu kecil → embedding tidak diskriminatif)
#
#  [5] use_classifier=False → ablation mode pure-reconstruction
#
#  PERBAIKAN BARU (v2) — Fix train_acc < test_acc:
#
#  [FIX-A] get_class_prototypes_from_embedding: prototype dibangun HANYA dari
#          baris observed (non-missing), bukan semua data termasuk yang missing.
#          Sebelumnya menggunakan semua ground-truth → data leakage pada test set
#          yang membuat test accuracy artifisial lebih tinggi dari training.
#
#  [FIX-B] get_eval: meneruskan observed_mask ke get_class_prototypes_from_embedding
#          agar prototype train dan test sama-sama bersih dari missing positions.
#
#  [FIX-C] alpha dinaikkan dari 0.1 → 0.3 (default) agar classifier loss
#          memberikan sinyal yang cukup untuk membentuk embedding yang lebih
#          diskriminatif, mengurangi gap train/test accuracy.
#
...
def train_supervised_embedding_model(
        ...
        hidden_dim: int = 256,
        patience: int = 40,
        alpha: float = 0.3,          # bobot classification — dinaikkan dari 0.1 agar embedding lebih diskriminatif
    ...
    alpha=0.3, beta=1.0: rekonstruksi mendominasi namun classifier loss cukup
    kuat untuk membentuk embedding yang diskriminatif per kelas.
    Sebelumnya alpha=0.1 terlalu kecil sehingga embedding tidak terstruktur
    per kelas → gap train/test accuracy membesar.
    """
    ...
    print(f'[Embedding] Mode: {mode_str} | alpha={alpha if use_classifier else 0} (target: ~0.3), beta={beta}')
...
def get_class_prototypes_from_embedding(
        ...
        device: str,
        observed_mask: np.ndarray = None,  # [N, n_cat_cols] bool, True = observed (non-missing)
) -> list:
    """
    ...
    Cara kerja (analog dengan nearest-prototype di original binary bits):
      Untuk kolom j, kelas c:
        proto[j][c] = mean( emb_recon[i, emb_j] ) untuk semua i di mana
                      true_label[i,j] == c  DAN  observed_mask[i,j] == True

    PERBAIKAN [FIX-A]: Prototype hanya dibangun dari posisi OBSERVED (non-missing).
    Sebelumnya menggunakan semua baris termasuk yang missing → prototype test set
    dipengaruhi ground-truth dari posisi missing → data leakage → test acc
    artifisial lebih tinggi dari training acc.
    ...
    for j in range(n_cols):
        ...
        # [FIX-A] Filter: hanya gunakan baris observed untuk membangun prototype
        if observed_mask is not None:
            obs_rows = observed_mask[:, j].astype(bool)
        else:
            obs_rows = np.ones(len(true_j), dtype=bool)

        for c in range(nclass):
            # Hanya ambil baris di mana label == c DAN posisi observed
            mask_c = (true_j == c) & obs_rows
            if mask_c.sum() > 0:
                proto_j[c] = emb_j[mask_c].mean(axis=0)
            else:
                # Fallback: jika tidak ada observed untuk kelas ini,
                # gunakan semua baris dengan label == c (tanpa filter observed)
                mask_c_all = (true_j == c)
                if mask_c_all.sum() > 0:
                    proto_j[c] = emb_j[mask_c_all].mean(axis=0)
...
def load_dataset(dataname, idx=0, mask_type='MCAR', ratio='30',
    ...
    --------------------------------
    emb_alpha         : bobot loss klasifikasi (default 0.3 — dinaikkan dari 0.1/0.2
                        agar embedding lebih diskriminatif, mengurangi gap train/test acc)
...
def get_eval(dataname, X_recon, X_true, truth_cat_idx,
    ...
    Cara kerja:
      1. Bangun prototype kelas j, kelas c =
             mean( emb_recon[i, col_j] )  untuk semua i dengan:
             - true_label[i,j] == c
             - observed_mask[i,j] == True  ← [FIX-B] hanya posisi OBSERVED
      2. pred[i,j] = argmin_c || emb_recon[i, col_j] - proto[j][c] ||^2
      3. Akurasi hanya dihitung pada posisi missing

    PERBAIKAN [FIX-B]: observed_mask diteruskan ke get_class_prototypes_from_embedding
    agar prototype tidak dibangun dari posisi missing → menghilangkan data leakage
    yang sebelumnya menyebabkan test accuracy artifisial > training accuracy.
    """
    info_path = f'datasets/Info/{dataname}.json'
    with open(info_path, 'r') as f:
        info = json.load(f)
    ...
    if (truth_cat_idx is not None
        ...
        # [FIX-B] Bangun observed_mask: posisi yang TIDAK missing (bukan yang missing)
        # Prototype hanya dibangun dari posisi observed agar tidak ada data leakage.
        # cat_mask adalah True di posisi missing → observed adalah kebalikannya.
        observed_cat_mask = ~cat_mask if cat_mask is not None else None

        # Bangun prototype dari ground-truth + embedding rekonstruksi
        # HANYA pada posisi observed (non-missing)
        prototypes = get_class_prototypes_from_embedding(
            ...
            device          = device,
            observed_mask   = observed_cat_mask,  # [FIX-B] filter observed saja
        )
