...
#
#  [BARU] Fitur numerik di-diskritisasi dengan ChiMerge lalu di-embed bersama
...
# ===========================================================================
#  ChiMerge Discretizer (implementasi dari scratch, tanpa dependency eksternal)
# ===========================================================================

class ChiMergeDiscretizer:
    """
    ChiMerge Discretizer.

    Implementasi algoritma Kerber (1992):
    - Bottom-up merging interval berdasarkan uji Chi-square
    - Stopping criterion: chi2 statistic < threshold (berdasarkan significance level)
    - Fit pada data training, transform ke integer bin index

    Referensi:
        Kerber, R. (1992). ChiMerge: Discretization of numeric attributes.
        In Proceedings of AAAI, pp. 123–128.
    """

    def __init__(self, significance_level: float = 0.05, min_bins: int = 2,
                 max_bins: int = 50, min_samples: int = 1):
        """
        Parameter
        ---------
        significance_level : float
            Level signifikansi uji Chi-square (default 0.05).
            Merge dilakukan jika chi2 < threshold pada level ini.
        min_bins : int
            Minimum jumlah bin yang dipertahankan (default 2).
        max_bins : int
            Maksimum jumlah bin (stopping criterion tambahan, default 50).
        min_samples : int
            Minimum jumlah sample per interval awal.
        """
    ...
    @staticmethod
    def _chi2_stat(freq_table: np.ndarray) -> float:
        """
        Hitung Chi-square statistic dari tabel frekuensi [2, n_classes].

        Chi2 = sum_i sum_j (O_ij - E_ij)^2 / E_ij
        E_ij = (row_sum_i * col_sum_j) / total
        """
        row_sum = freq_table.sum(axis=1, keepdims=True)   # [2, 1]
        col_sum = freq_table.sum(axis=0, keepdims=True)   # [1, n_classes]
        total   = freq_table.sum()

        if total == 0:
            return 0.0

        expected = (row_sum * col_sum) / total             # [2, n_classes]

        # Hindari pembagian nol: E_ij = 0 → kontribusi = 0
        with np.errstate(divide='ignore', invalid='ignore'):
            chi2 = np.where(
                expected > 0,
                (freq_table - expected) ** 2 / expected,
                0.0
            )
        return float(chi2.sum())

    @staticmethod
    def _build_freq_table(y1: np.ndarray, y2: np.ndarray,
                          classes: np.ndarray) -> np.ndarray:
        """
        Bangun tabel frekuensi [2, n_classes] dari dua interval.
        """
        n_classes = len(classes)
        table = np.zeros((2, n_classes), dtype=np.float64)
        for k, c in enumerate(classes):
            table[0, k] = np.sum(y1 == c)
            table[1, k] = np.sum(y2 == c)
        return table

    # ── Public API ───────────────────────────────────────────────────────

    def fit(self, X: np.ndarray, y: np.ndarray) -> 'ChiMergeDiscretizer':
        """
        Fit ChiMerge pada data training.
        ...
        n_cols  = X.shape[1]
        classes = np.unique(y)
        df      = len(classes) - 1           # degrees of freedom untuk chi2
        df      = max(df, 1)

        # Threshold chi2: merge jika chi2 < threshold
        chi2_threshold = chi2.ppf(1.0 - self.significance_level, df=df)
        ...
        for col in range(n_cols):
            ...
            sort_idx = np.argsort(x_valid, kind='stable')
            x_sorted = x_valid[sort_idx]
            y_sorted = y_valid[sort_idx]

            # ── Inisialisasi: setiap nilai unik menjadi satu interval ────
            unique_vals, inv_idx = np.unique(x_sorted, return_inverse=True)
            n_intervals = len(unique_vals)

            # intervals: list of (list_of_y_values) per interval
            intervals = []
            for i in range(n_intervals):
                mask = inv_idx == i
                intervals.append(y_sorted[mask])

            # ── Bottom-up merging ─────────────────────────────────────────
            while len(intervals) > self.min_bins:
                # Hitung chi2 antara setiap pasangan interval bersebelahan
                chi2_vals = []
                for i in range(len(intervals) - 1):
                    freq = self._build_freq_table(intervals[i], intervals[i + 1], classes)
                    chi2_vals.append(self._chi2_stat(freq))

                min_chi2 = min(chi2_vals)

                # Stopping: jika chi2 minimum >= threshold, stop merging
                if min_chi2 >= chi2_threshold and len(intervals) <= self.max_bins:
                    break

                # Merge interval dengan chi2 terkecil (paling mirip)
                merge_idx = int(np.argmin(chi2_vals))
                merged_y  = np.concatenate([intervals[merge_idx],
                                             intervals[merge_idx + 1]])
                intervals = (intervals[:merge_idx]
                             + [merged_y]
                             + intervals[merge_idx + 2:])

            # ── Hitung cut points dari interval final ─────────────────────
            # Cut point = titik tengah antara nilai max interval ke-i dan
            # nilai min interval ke-(i+1), dalam skala nilai asli.
            # Perlu mapping kembali: boundaries antara interval bersebelahan.

            # Rebuild unique values per interval dari sort
            interval_boundaries = []
            ptr = 0
            for i, ivl in enumerate(intervals):
                n_i = len(ivl)
                x_slice = x_sorted[ptr: ptr + n_i]
                interval_boundaries.append((x_slice.min(), x_slice.max()))
                ptr += n_i

            cuts = []
            for i in range(len(interval_boundaries) - 1):
                cut = (interval_boundaries[i][1] + interval_boundaries[i + 1][0]) / 2.0
                cuts.append(cut)
            ...
            print(f'  [ChiMerge] Col {col}: {len(cuts)} cut points → {len(cuts)+1} bins')
...
class SupervisedLearnableEmbeddingModel(nn.Module):
    ...
    Sekarang juga dipakai untuk fitur numerik yang sudah di-diskritisasi ChiMerge.
...
def decode_num_from_embedding(model: SupervisedLearnableEmbeddingModel,
    ...
    Alur (Soft Expected Value):
      embedding → linear decoder → logits per bin → softmax → probabilitas
      → expected value = sum(p_k * midpoint_k) untuk setiap kolom numerik

    Soft Expected Value memberikan estimasi yang lebih halus dibanding argmax
    karena memanfaatkan distribusi probabilitas penuh dari logits, bukan hanya
    bin dengan nilai tertinggi.
    ...
    with torch.no_grad():
        for (batch,) in loader:
            recon_logits = model.decode(batch)   # list[n_cols] of [B, vocab_i]

            batch_num_pred = []
            for col in range(n_num_cols):
                logits_col = recon_logits[col]          # [B, n_bins_col]
                probs      = torch.softmax(logits_col, dim=1)  # [B, n_bins_col]

                mids       = torch.tensor(
                    bin_midpoints[col], dtype=torch.float32, device=device
                )  # [n_bins_col]

                # Expected value: sum_k p_k * midpoint_k
                # probs: [B, n_bins_col], mids: [n_bins_col] → [B]
                exp_val = (probs * mids.unsqueeze(0)).sum(dim=1)  # [B]
                batch_num_pred.append(exp_val.unsqueeze(1))       # [B, 1]

            # stack: [B, n_num_cols]
            batch_num_pred = torch.cat(batch_num_pred, dim=1)
            all_num_pred.append(batch_num_pred.cpu().numpy())

    return np.concatenate(all_num_pred, axis=0).astype(np.float32)
...
def load_dataset(dataname, idx=0, mask_type='MCAR', ratio='30'):
    """
    Load dataset dengan ChiMerge discretization untuk numerik +
    Supervised Embedding untuk SEMUA kolom (numerik-bin + kategorikal).

    Perubahan dari versi sebelumnya:
    - Fitur numerik di-diskritisasi dengan ChiMerge → integer bin index
    ...
    - chimerge      : ChiMergeDiscretizer  (untuk transform test & decode)
    ...
    emb_sizes         : list[int]
    chimerge          : ChiMergeDiscretizer  (atau None jika tidak ada fitur numerik)
    ...
    if n_num_cols > 0:
        ...
        print(f'[ChiMerge] Menjalankan ChiMerge discretization pada {n_num_cols} kolom numerik ...')
        chimerge = ChiMergeDiscretizer(significance_level=0.05, min_bins=2, max_bins=50)

        # Fit ChiMerge pada train dengan label → transform train & test
        # ChiMerge difit pada nilai RAW (bukan normalisasi) untuk konsistensi cut point
        chimerge.fit(train_num_raw, train_labels)

        train_num_bin = chimerge.transform(train_num_raw)   # [N_train, n_num_cols] int64
        test_num_bin  = chimerge.transform(test_num_raw)    # [N_test,  n_num_cols] int64

        # Hitung bin midpoints dalam skala NORMALISASI
        # (dipakai saat decoding: Soft Expected Value menggunakan midpoints ini)
        bin_midpoints = chimerge.get_bin_midpoints(train_num_norm, train_num_bin)

        print(f'[ChiMerge] n_bins per kolom: {chimerge.n_bins_}')
        print(f'[ChiMerge] Total bins: {sum(chimerge.n_bins_)}')
    ...
    all_dims = (chimerge.n_bins_ if chimerge is not None else []) + cat_dims_cat
    ...
    # Sudah dihitung di atas, disimpan di chimerge.bin_midpoints_ & bin_midpoints
    ...
    return (train_X, test_X,
            ...
            emb_model,
            emb_sizes,
            chimerge,        # [BARU] ChiMergeDiscretizer
