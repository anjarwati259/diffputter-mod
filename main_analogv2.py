...
from dataset_analogv2 import load_dataset, get_eval, mean_std, _analog_bits_to_int
...
if __name__ == '__main__':
    ...
    for iteration in range(args.max_iter):
        ...
        rec_X_stack = torch.stack(rec_Xs, dim=0)  # (num_trials, N, dim)

        # Numerik: tetap pakai mean antar trial
        rec_X = rec_X_stack.mean(0)

        # Kategorik: majority voting per kolom kategorik
        len_num = train_num.shape[1]
        pred_cat_idx_voted = None  # hasil voting integer index, untuk evaluasi langsung
        if cat_bin_num is not None and len(cat_bin_num) > 0:
            cat_bin_num_arr = np.array(cat_bin_num).astype(int)
            ends   = np.cumsum(cat_bin_num_arr)
            starts = np.concatenate(([0], ends[:-1]))

            cat_stack = rec_X_stack[:, :, len_num:].cpu().numpy()  # (num_trials, N, cat_bits)

            N = rec_X.shape[0]
            pred_cat_idx_voted = np.zeros((N, len(cat_bin_num)), dtype=int)

            for j, (s, e) in enumerate(zip(starts, ends)):
                bits_per_trial = cat_stack[:, :, s:e]  # (num_trials, N, b)

                # Decode tiap trial ke integer index
                trial_preds = np.stack([
                    _analog_bits_to_int(bits_per_trial[t]) for t in range(num_trials)
                ], axis=0)  # (num_trials, N)

                # Majority vote per baris
                voted = np.squeeze(stats.mode(trial_preds, axis=0)[0]).flatten().astype(int)  # (N,)

                # Simpan hasil voting untuk evaluasi langsung (Pilihan B)
                pred_cat_idx_voted[:, j] = voted

                # Re-encode voted index ke skala ternormalisasi (konsisten dengan X)
                b = e - s
                for bit_pos in range(b):
                    col_idx = len_num + s + bit_pos
                    bit_val = ((voted >> (b - 1 - bit_pos)) & 1).astype(np.float32)
                    analog_val = bit_val * 2.0 - 1.0                                       # {-1, +1}
                    analog_val_norm = (analog_val - mean_X[col_idx]) / std_X[col_idx] / 2  # skala X
                    rec_X[:, col_idx] = torch.tensor(
                        analog_val_norm.astype(np.float32), dtype=torch.float32, device=device
                    )
        ...
        # Denormalisasi di GPU (hanya bagian kategorik)
        ...
        # Pilihan B: pass pred_cat_idx_voted langsung ke get_eval (bypass decode kontinu)
        mae, rmse, acc = get_eval(dataname, pred_X, X_true, train_cat_idx, train_num.shape[1], cat_bin_num, ori_train_mask, pred_cat_idx=pred_cat_idx_voted)
        MAEs.append(mae)
        RMSEs.append(rmse)
        ACCs.append(acc)
        ...
        # Kategorik: majority voting per kolom kategorik
        len_num = train_num.shape[1]
        pred_cat_idx_voted_oos = None  # hasil voting integer index, untuk evaluasi langsung
        if cat_bin_num is not None and len(cat_bin_num) > 0:
            cat_bin_num_arr = np.array(cat_bin_num).astype(int)
            ends   = np.cumsum(cat_bin_num_arr)
            starts = np.concatenate(([0], ends[:-1]))
            ...
            N_oos = rec_X.shape[0]
            pred_cat_idx_voted_oos = np.zeros((N_oos, len(cat_bin_num)), dtype=int)
            ...
            for j, (s, e) in enumerate(zip(starts, ends)):
                ...
                # Simpan hasil voting untuk evaluasi langsung (Pilihan B)
                pred_cat_idx_voted_oos[:, j] = voted

                # Re-encode voted index ke skala ternormalisasi (konsisten dengan X_test)
                b = e - s
                for bit_pos in range(b):
                    col_idx = len_num + s + bit_pos
                    bit_val = ((voted >> (b - 1 - bit_pos)) & 1).astype(np.float32)
        ...
        # Pilihan B: pass pred_cat_idx_voted langsung ke get_eval (bypass decode kontinu)
        mae_out, rmse_out, acc_out = get_eval(dataname, pred_X, X_true, test_cat_idx, test_num.shape[1], cat_bin_num, ori_test_mask, oos=True, pred_cat_idx=pred_cat_idx_voted_oos)
        ...
        with open(f'{result_save_path}/result_analog.txt', 'a+') as f:
