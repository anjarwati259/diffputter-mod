...
from dataset_v2nn import (load_dataset, get_eval, mean_std,
...
if __name__ == '__main__':
    ...
    for iteration in range(args.max_iter):
        ...
        if iteration == 0:
            ...
            # Gunakan SELURUH hasil rekonstruksi iterasi sebelumnya (termasuk
            # posisi observed) sebagai input M-step — sama dengan pola main.py.
            # Ini menciptakan "snowball effect": model di iterasi berikutnya belajar
            # dari data yang makin lengkap dan halus, sehingga akurasi naik tiap iter.
            X_miss     = torch.tensor(
        ...
        for trial in tqdm(range(num_trials), desc='In-sample imputation'):
            ...
            # Blend: posisi missing → imputasi diffusion, posisi observed → X asli.
            # Catatan: impute_X sudah berisi X asli di posisi observed (bukan 0),
            # sehingga blend ini mempertahankan nilai observed yang benar.
        ...
        for trial in tqdm(range(num_trials), desc='Out-of-sample imputation'):
            ...
            # Blend: observed → X_test asli (via impute_X), missing → imputasi
        ...
        with open(f'{result_save_path}/result_fullv2.txt', 'a+') as f:
            f.write(
