...
from dataset_vae2 import (load_dataset, get_eval, 
                         normalize_for_diffusion, denormalize_all)
...
parser.add_argument('--norm_method', type=str, default='standard', 
                   ...
                   help='Normalization method: standard (z-score) or robust (median-IQR)')
...
if __name__ == '__main__':
    ...
    # Unified normalization untuk semua features
    # Ini memastikan range yang consistent untuk diffusion model
    num_num = train_num.shape[1]
    
    train_X_norm, test_X_norm, norm_params = normalize_for_diffusion(
        train_X, test_X, train_mask, test_mask, 
        num_num=num_num,
    ...
    print(f"Normalized data range: [{train_X_norm.min():.2f}, {train_X_norm.max():.2f}]")
    
    in_dim = train_X_norm.shape[1]

    X = torch.tensor(train_X_norm, dtype=torch.float32)
    X_test = torch.tensor(test_X_norm, dtype=torch.float32)
    ...
    for iteration in range(args.max_iter):
        ...
        if iteration == 0:
            ...
            X_miss_raw = np.load(f'{ckpt_dir}/iter_{iteration}.npy')
            # Re-normalize dengan parameters yang sama
            X_miss, _, _ = normalize_for_diffusion(
                X_miss_raw, X_miss_raw,
                train_mask, train_mask,
                num_num=num_num,
                norm_method=norm_params['method'],
                clip_range=norm_params['clip_range']
            )
            train_data = X_miss
            print(f'[INFO] Loaded X_miss shape: {train_data.shape}, range: [{train_data.min():.4f}, {train_data.max():.4f}]')
        ...
        # Denormalize SEMUA features (simple!)
        rec_X_denorm = denormalize_all(rec_X_np, norm_params)
        X_true_denorm = denormalize_all(X_true_np, norm_params)

        # Save untuk next iteration
        np.save(f'{ckpt_dir}/iter_{iteration+1}.npy', rec_X_denorm)

        # Evaluate - fungsi get_eval akan split sendiri:
        # - Numerical: RMSE/MAE di original scale
        # - Categorical: VAE decoder → accuracy
        mae, rmse, acc = get_eval(
            dataname, rec_X_denorm, X_true_denorm,
        ...
        # Denormalize SEMUA features
        rec_X_denorm = denormalize_all(rec_X_np, norm_params)
        X_true_denorm = denormalize_all(X_true_np, norm_params)

        # Evaluate - split dilakukan di get_eval
