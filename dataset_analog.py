...
def get_eval(dataname, X_recon, X_true, truth_cat_idx, num_num, cat_bin_num, mask, oos=False):
    ...
    Logika Accuracy — Analog Bits Decoding:
    ----------------------------------------
    Paper menyebutkan: "we convert every output element to 1 if the output
    element is larger than 0, otherwise we convert it to -1."

    Implementasi (sesuai `_analog_bits_to_int`):
        1. output > 0  →  bit = 1  |  output ≤ 0  →  bit = 0
        2. bit string → integer index
        3. bandingkan dengan ground-truth label index (truth_cat_idx)

    Threshold 0 (bukan 0.5) karena analog bits encoding simetris di nol.
    """
    ...
    if (truth_cat_idx is not None) and (len(cat_col_idx) > 0) and (cat_bin_num is not None):
        ...
        for j, (s, e) in enumerate(zip(starts, ends)):
            ...
            # Analog bits output diffusion untuk kolom kategorik ke-j
            ...
            # ---------------------------------------------------------------
            # DECODE ANALOG BITS → integer index (threshold = 0)
            # ---------------------------------------------------------------
