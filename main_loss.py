...
from dataset_loss import (load_dataset, get_eval, mean_std,
...
def composite_loss(pred, target, len_num, emb_model, emb_sizes,
                   mean_gpu, std_gpu, lambda_cat=0.5):
    """
    Composite loss untuk diffusion training:
      - Kolom numerik  : MSE (sama seperti asli)
      - Kolom kategorik: Cross-Entropy via linear decoder emb_model

    Parameter
    ---------
    pred / target : [B, num_num + total_emb_dim]  — skala ternormalisasi /2
    len_num       : jumlah kolom numerik
    emb_model     : SupervisedLearnableEmbeddingModel (untuk decode embedding → logits)
    emb_sizes     : list[int]  dimensi embedding per kolom kategorik
    mean_gpu, std_gpu : tensor normalisasi  [num_num + total_emb_dim]
    lambda_cat    : bobot CE loss relatif terhadap MSE loss
    """
    # ── Numerik: MSE ────────────────────────────────────────────────────────
    loss_num = F.mse_loss(pred[:, :len_num], target[:, :len_num])

    # ── Kategorik: CE via linear decoder ────────────────────────────────────
    if emb_model is None or len(emb_sizes) == 0:
        return loss_num

    # Denormalisasi bagian embedding kembali ke skala emb_model
    # pred/target dalam skala /2, jadi: emb_asli = x * 2 * std + mean
    pred_emb   = pred[:, len_num:]   * 2 * std_gpu[len_num:] + mean_gpu[len_num:]
    target_emb = target[:, len_num:] * 2 * std_gpu[len_num:] + mean_gpu[len_num:]

    # Decode prediksi embedding → logits per kolom kategorik
    recon_logits_pred   = emb_model.decode(pred_emb)    # list[n_cat] of [B, vocab_i]

    # Ground-truth kelas: argmax dari embedding target yang sudah di-decode
    # (target_emb merupakan embedding observasi asli yang dilatih oleh emb_model)
    recon_logits_target = emb_model.decode(target_emb)  # list[n_cat] of [B, vocab_i]
    true_classes = [
        torch.argmax(lgt, dim=1)                        # [B]  — hard label
        for lgt in recon_logits_target
    ]

    # Hitung CE untuk tiap kolom, rata-rata
    loss_cat = sum(
        F.cross_entropy(recon_logits_pred[j], true_classes[j])
        for j in range(len(emb_sizes))
    ) / len(emb_sizes)

    return loss_num + lambda_cat * loss_cat
    ...
    for iteration in range(args.max_iter):
        ...
        for epoch in pbar:
            ...
            for batch in train_loader:
                ...
                # ── Diffusion loss utama (MSE, tidak diubah) ─────────────────
                loss_diffusion = model(inputs).mean()

                # ── Tambahan CE loss untuk kolom kategorik (embedding) ────────
                # Hanya aktif jika ada emb_model dan kolom kategorik
                if emb_model is not None and len_num < inputs.shape[1]:
                    # Prediksi x0 dari net dengan noise ringan (t=1, hampir bersih)
                    # Tujuan: ukur seberapa baik model memprediksi kelas kategorik
                    net     = model.denoise_fn_D
                    t_val   = torch.ones(inputs.shape[0],
                                         device=inputs.device, dtype=torch.long)
                    x_noisy = inputs + torch.randn_like(inputs) * 0.1
                    pred_x0 = net(x_noisy, t_val)

                    # Hitung CE loss kategorik dari prediksi embedding → logits
                    loss_ce_cat = composite_loss(
                        pred       = pred_x0,
                        target     = inputs,
                        len_num    = len_num,
                        emb_model  = emb_model,
                        ...
                        mean_gpu   = mean_X_gpu,
                        std_gpu    = std_X_gpu,
                        lambda_cat = 0.5,   # bisa di-tune: 0.1, 0.5, 1.0
                    )
                    # Kurangi MSE numerik agar tidak double-count dengan loss_diffusion
                    loss_mse_num = F.mse_loss(pred_x0[:, :len_num], inputs[:, :len_num])
                    loss_ce_only = (loss_ce_cat - loss_mse_num).clamp(min=0.0)

                    loss = loss_diffusion + loss_ce_only
                else:
                    # Dataset tanpa kategorik: pakai diffusion loss murni
                    loss = loss_diffusion
        ...
        with open(f'{result_save_path}/result_impr1.txt', 'a+') as f:
            f.write(
